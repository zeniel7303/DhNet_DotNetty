...
### 2026-04-03 — 보안 버그 수정: BCrypt 비동기 분리 + 이동 좌표 속도 검증

전체 코드 리뷰(Opus)에서 발견된 두 가지 보안/안정성 이슈를 수정했다.

#### RegisterProcessor — BCrypt.HashPassword Task.Run 분리

`LoginProcessor`는 이미 `BCrypt.Verify`를 `Task.Run()`으로 분리해 DotNetty I/O 스레드 블로킹을 피하고 있었으나,
`RegisterProcessor`는 `BCrypt.HashPassword`(~200ms)를 I/O 스레드에서 동기 호출하고 있었다.

두 프로세서 간 일관성 부재를 수정했다:
...
회원가입 빈도가 낮아 실제 처리량 영향은 미미하지만, 단 한 번의 해싱이라도 I/O 이벤트 루프를 점유하면 같은 루프에 연결된 다른 세션의 패킷 처리가 지연된다.

#### PlayerWorldComponent — TryMove 이동 속도 검증

클라이언트가 보낸 좌표를 서버가 무조건 신뢰하던 구조를 수정했다.
`TryMove(x, y)` 메서드를 추가해 마지막 이동 이후 경과 시간 기반으로 최대 이동 거리를 검증한다:

```
최대 허용 거리 = MoveSpeed × elapsed(최대 1.0s) × 1.8(지터 여유)
```

맵 순환 경계 점프(`|dx| ≥ MapW * 0.5`)는 검증 제외 — 기존 wrap-around 처리와 일관성 유지.
검증 실패 시 이동을 드랍하고 경고 로그를 남긴다(연결 종료는 하지 않음 — 클라이언트 지터로 인한 오탐 가능성 고려).

`Move()`는 `private`으로 변경해 외부에서 검증 우회가 불가능해졌다.

| 항목 | 수정 전 | 수정 후 |
|------|---------|---------|
| BCrypt.HashPassword | I/O 스레드 동기 호출 | `await Task.Run()` ThreadPool 분리 |
| 이동 좌표 검증 | 없음 (클라이언트 전적 신뢰) | `TryMove` — 속도 기반 텔레포트 감지 |
