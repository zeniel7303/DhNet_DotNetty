...
public class PasswordResetTokenDbSet
{
    ...
    public Task<int> InsertAsync(PasswordResetTokenRow row, CancellationToken cancellationToken = default)
    {
        const string sql = @"
    ...
    public Task<PasswordResetTokenRow?> SelectByTokenAsync(string token, CancellationToken cancellationToken = default)
    {
        const string sql = @"
            ...
            WHERE `token` = @token
            LIMIT 1";
        return _conn.QuerySingleOrDefaultAsync<PasswordResetTokenRow>(sql, new { token }, cancellationToken);
    }
    ...
    /// </summary>
    public Task<int> MarkUsedConditionalAsync(ulong tokenId, CancellationToken cancellationToken = default)
    {
        const string sql = @"
        ...
        return _conn.ExecuteAsync(sql, new { tokenId }, cancellationToken);
    }
    ...
    public Task<int> DeleteExpiredAsync(CancellationToken cancellationToken = default)
    {
        ...
        return _conn.ExecuteAsync(sql, cancellationToken: cancellationToken);
    }
}
