...
[Browser / 관리 도구] ──HTTP:8080──▶ [ASP.NET Core Web Server]
                                           ...
                                           POST /rooms/{id}/broadcast
                                                │ In-Process
                                     PlayerSystem / LobbySystem / LobbyComponent
```
...
### 6. GamePacket oneof 래퍼 — 16종 패킷 단일 타입 처리
...
│
├── GameServer.Protocol/
│   └── Protos/                      .proto 5개 → C# 자동 생성 (Grpc.Tools)
│       ├── game_packet.proto        GamePacket oneof 래퍼 (16종)
...
│   │
│   └── Web/
│       ├── WebServerHost.cs             Kestrel 호스트 빌더, CancellationToken 연동
│       ├── Dtos.cs                      HealthDto, LobbyDto, RoomDto, BroadcastBody
│       └── Controllers/
│           ├── HealthController.cs      GET /health → { "status": "OK" }
│           ├── LobbyController.cs       GET /lobby → { "playerCount": N }
│           └── RoomsController.cs       GET /rooms, POST /rooms/{id}/broadcast
│
├── GameClient/
...
│   └── Stats/
│       └── LoadTestStats.cs         Interlocked 카운터 (Sent/Recv/Errors/Connected/Disconnected/Reconnects/RoomCycles)
│
├── db/
...
`GameServer/appsettings.json` DB 접속 정보 설정:
...
```
Active=42 | Sent=12840 | Recv=11203 | Errors=0 | Reconnects=980 | RoomCycles=490
```
