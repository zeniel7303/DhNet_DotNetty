...
// Players
public record PlayerDto(ulong PlayerId, string Name, ulong? LobbyId, ulong? RoomId);

// Lobbies
public record LobbyDetailDto(ulong LobbyId, int PlayerCount, int MaxCapacity, bool IsFull);

// Room detail
public record PlayerInRoomDto(ulong PlayerId, string Name);
...
// Analytics
public record ChatLogDto(ulong PlayerId, ulong? RoomId, string Channel, string Message, DateTime CreatedAt);
public record LoginLogDto(ulong PlayerId, string PlayerName, string? IpAddress, DateTime LoginAt, DateTime? LogoutAt);
public record RoomLogEntryDto(ulong PlayerId, ulong RoomId, string Action, DateTime CreatedAt);
