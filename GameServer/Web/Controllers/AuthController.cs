...
public class AuthController(SmtpService smtp, IConfiguration config) : ControllerBase
{
    ...
    public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordRequest req)
    {
        ...
        try
        {
            ...
            await DbGateway.Current.DeleteExpiredPasswordResetTokensAsync(ct);
            ...
            var account = await DbGateway.Current.FindAccountByUsernameAsync(req.Username.Trim(), ct);
            if (account == null
        ...
        {
            GameLogger.Error("Auth", "비밀번호 재설정 요청 시간 초과", ex);
    ...
    public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordRequest req)
    {
        ...
        try
        {
            ...
            var row = await DbGateway.Current.FindPasswordResetTokenAsync(req.Token.Trim(), ct);
            ...
            var marked = await DbGateway.Current.ConsumePasswordResetTokenAsync(row.token_id, ct);
            if (marked == 0)
                return BadRequest(new { error = "유효하지 않거나 만료된 토큰입니다." });
            ...
            await DbGateway.Current.UpdatePasswordHashAsync(row.account_id, newHash, ct);
        ...
        {
            GameLogger.Error("Auth", "비밀번호 재설정 시간 초과", ex);
            return DatabaseTimeout();
        }
    }

    private static IActionResult DatabaseTimeout()
        => new ObjectResult(new { error = "Database timeout" }) { StatusCode = 503 };
}
